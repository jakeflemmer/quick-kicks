...
{ 
	...
	public class CPUTeamController extends AbstractTeamController 
	{
		...
		{
			keeper = new CPUPlayerController(true);
			player1 = new CPUPlayerController();
			player2 = new CPUPlayerController();
			player3 = new CPUPlayerController();
			player4 = new CPUPlayerController();	
		...
		public function reset():void
		{
			...
			initializePlayerPositions();						
			noPlayersGettingBall( playersArray );			
...
//				}
				...
				//trace('cpu team controller - pwbd.justPassed : ' + pwbd.justPassed);
				
				if ( _data.pressureToPass && pwbd.justPassed == false)
				{		
...
//  _______________________________________________   sharp shooter   _________________________________________________________
	...
	private function keeperTracking():void
	{
		...
		var deltaX:Number = ( 1450 - sbd.xPos );  // 1450 = goalCenterX
		var deltaY:Number = ( 500 - sbd.yPos );  //  500 = goalCenterY
		
		var theAngleFromGoal:Number = RadiansCalculator.calc(deltaX,deltaY);
		var keeperDestinationX:Number = (1475 - ( 150 * Math.cos(theAngleFromGoal)));
	...
	protected function noPlayersMarking( arrayOfPlayers:Array ):void{
		for ( var i:int = 0 ; i < arrayOfPlayers.length ; i ++ )
		{
			arrayOfPlayers[i].data.marking = false;		
		}
		//trace("p1marking : " + player1.data.marking + "pwb : " + pwbd.marking);
	}	
